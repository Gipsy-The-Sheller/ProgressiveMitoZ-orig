...
def filter_by_abundance(in_sim=None, in_fasta=None, abundance_pattern=None, min_abundance=10):
    ...
    with open(in_sim, 'r') as fh:
        for i in fh:
            ...
            line = i.split('\t')
            seqid = line[0]
            m = re.search(abundance_pattern, line[-1])
            try:
                abun = m.group(1)
            except AttributeError:
                print("Failed to extract the abundance information from the seqid!", file=sys.stderr)
                print("""You may need to change the value of '--abundance_pattern' option,
or set '--min_abundance 0' if you do NOT want to filter sequences by abundance!""", file=sys.stderr)
                sys.exit(0)
...
def main():
    usage = '''
python3 {0} <abundance_pattern> <min_abundance> <work71.hmmtblout.besthit.sim[.filtered]> <in.fasta>
'''.format(sys.argv[0])

    if len(sys.argv) != 5:
        sys.exit(usage)

    abundance_pattern, min_abundance, in_sim, in_fasta = sys.argv[1:5]
    filter_by_abundance(
