...
def get_rank_dict(taxa_name=None):
	...
	if not name_dict:
		...
		if not name_dict:
			...
			return None
	# name_dict: {'Homo sapiens': [9606]} # the same species name can have multiple taxids!
...
def get_rank_dict2(taxa_name=None):
	...
	ranks = ('user_taxa', 'taxa_searched', 'kingdom','phylum', 'class', 'order', 'family', 'subfamily', 'genus', 'species')
	for potential_taxid in rank_taxon.lineages:
		kingdom, kingdom_taxid = rank_taxon.lineages[potential_taxid]['kingdom']
		if kingdom != 'Metazoa':
			print('Found non-Metazoa, skip over it! kingdom: {kingdom}, taxid: {taxid}'.format(kingdom=kingdom, taxid=potential_taxid), file=sys.stderr)
			continue
		for rank in ranks:
			taxa = rank_taxon.lineages[potential_taxid][rank][0]
			rank_dict[rank] = taxa
...
def filter_taxa(annt_fa=None, cdspos=None, outf=None, requiring_taxa=None):
	...
	requiring_rank_dict = get_rank_dict2(taxa_name=requiring_taxa)
	...
	with open(cdspos, 'r') as fh, open(cdspos_taxa_log, 'w') as fh_taxalog:
		...
		for i in fh:
			...
			if match:
				...
				rank_dict = get_rank_dict2(taxa_name=species)
				if not rank_dict:
					print(file=fh_taxalog)
					continue
...
if __name__ == "__main__":
	...
	cdsposcds_file = mito_cds_annt(MTsoft=args.MTsoft, annt_fa=args.fa,
		db=args.db, thread=args.thread, genetic_code=args.genetic_code)
